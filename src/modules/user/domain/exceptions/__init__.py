# /src/modules/user/domain/exceptions/__init__.py
...
class EmailAlreadyExistsException(BaseHTTPException):
    """
    Class responsible for handling exceptions when an
        email is already registered.

    This exception is raised when a user tries to create an account using an
        email that is already associated with an existing account.
...
class InvalidUserRemovalException(BaseHTTPException):
    """
    Class responsible for handling exceptions when the process of removing
        a user is invalid or incorrect.
