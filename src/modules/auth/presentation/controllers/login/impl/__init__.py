# /src/modules/auth/presentation/controllers/login/__init__.py
...
# Domain
from src.modules.auth.presentation.controllers.login import ILoginController
...
class LoginControllerImpl(ILoginController):
    ...
    This class provides an endpoint to authenticate users,
    ...
    def __init__(
        ...
        """
        Constructor method that initializes the LoginController
    ...
    def __call__(self, body: LoginRequestDTO) -> JSONResponse:
        """
        Public method that authenticates a user based
            on the provided credentials.

        Args:
            body (AuthenticationRequestDTO): Data Transfer Object (DTO)
                containing the user's login
                credentials (e.g., email and password).

        Returns:
            JSONResponse: A JSON response containing an
